               ...
               <div className="rounded-lg shadow md:max-w-xl p-4">
                   {user ? (
                       <div className="flex items-center m-auto justify-center">
                           <img src={user.avatar} alt="Avatar del usuario" className="w-12 h-12 rounded-full ml-3" />
                           <span className="font-koulen ml-3 text-white">{user.name}</span>
                       </div>
                   ) : (
                       <p className="text-white">No se pudo encontrar al usuario asociado a este articulo.</p>
                   )}
                 </div>
            </div>
            <h5 className="p-4 mb-2 text-2xl font-koulen tracking-tight text-white">{articulo.nombre}</h5>
            <div className="flex flex-col justify-between p-4 leading-normal">
                <ul className="mb-2 text-white">
                    {categorias.length > 0 ? (
                        categorias.map((categoria) => (
                            <li key={categoria.id} className="font-koulen">{categoria.nombre}</li>
                        ))
                    ) : (
                ...
                <ul className="mb-2 text-white">
                    {etiquetas.length > 0 ? (
                        etiquetas.map((etiqueta) => (
                            <li key={etiqueta.id} className="font-koulen">{etiqueta.nombre}</li>
                        ))
                    ) : (
                         <li>Sin etiquetas</li>
                    )}
                ...
                <ul className="text-white">
                    <li className="font-lato">Creada en <b>{articulo.created_at}</b></li>
                 </ul>
             </div>
            <div className="flex flex-col items-center w-full shadow md:flex-row p-5 m-3 justify-center">
                <ul className="text-center">
                    <li className="mb-3 font-lato text-white">{articulo.descripcion}</li>
                    <a href=""><li className="font-lato p-2 mt-2 bg-red-700 w-full w-1/3 text-center rounded-lg text-white text-sm">Comprar</li></a>
                 </ul>
             </div>
            </div>
            </div>
