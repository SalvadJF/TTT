...
export default function Index({ auth, user, articulos, comentarios, facturas }) {
    ...
    const currentItems = (category) => {
        if (category === "articulos") {
            ...
            return filteredFacturas.slice(indexOfFirstItem(), indexOfLastItem());
        }
    };
                ...
                >
                    ...
                    >
                        ...
                        {currentItems("facturas").map(factura => (
                            <div key={factura.id} className="mt-4 p-2 bg-white dark:bg-gray-800 mb-2 rounded-md shadow-md">
                                <Link href={`/facturas/${factura.id}`} className="text-blue-600 dark:text-blue-400">
                                    Factura {factura.id}
                                </Link>
                            </div>
                        ))}
                        ...
                        <div className="flex justify-center mt-4">
                            {Array.from({ length: Math.ceil(filteredFacturas.length / itemsPerPage) }).map((_, index) => (
                                <button key={index} onClick={() => paginate(index + 1 , "facturas")} className={`px-3 py-1 mx-1 rounded-md ${currentPage === index + 1 ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}>
                                    {index + 1}
                                </button>
                            ))}
                        </div>
