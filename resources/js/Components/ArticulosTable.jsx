            ...
            <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
                <table className="w-full text-sm text-center rtl:text-right text-gray-500 dark:text-gray-400">
                    <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            <th scope="col" className="px-6 py-3">ID del Articulo</th>
                    ...
                    <tbody>
                        {currentItems.map((articulo) => (
                            <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700" key={articulo.id}>
                                <th scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{articulo.id}</th>
                                <td className="px-6 py-4">
                                    <a href={`/articulos/${articulo.id}`} className="text-blue-600">{articulo.nombre}</a>
                                </td>
                                <th scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{articulo.user.name}</th>
                                <td className="px-6 py-4">{formatDate(articulo.created_at)}</td>
                                <td className="px-6 py-4 text-center">
                                    <a href={`/articulos/${articulo.id}/edit`} className="font-medium text-blue-600 dark:text-blue-500 hover:underline m-2">
                                        <button type="button" className="inline-flex items-center px-3 py-2 text-sm font-semibold border border-transparent rounded-lg gap-x-2 bg-no-aprobada text-neutro-4 hover:bg-blue-400 disabled:opacity-50 disabled:pointer-events-none">
                                            <img src="/img/iconos/edit.svg" alt="Icono Editar" className="w-4 h-4" />
                                        </button>
                                    </a>
                                    <button type="button" onClick={() => handleShowDeleteModal(articulo)} className="inline-flex items-center px-3 py-2 text-sm font-semibold border border-transparent rounded-lg gap-x-2 bg-no-aprobada text-neutro-4 hover:bg-red-400 disabled:opacity-50 disabled:pointer-events-none">
            ...
            {showDeleteModal && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="bg-white p-4 rounded-lg">
                        <p className="text-lg font-semibold mb-4">¿Estás seguro de que quieres borrar este artículo?</p>
