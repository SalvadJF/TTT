<x-app-layout>
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('profile.index') }}" class="font-koulen ms-1 text-sm font-medium text-white hover:text-blue-600 md:ms-2 dark:text-white dark:hover:text-white">{{ $user->name }}</a>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="flex items-center gap-4 m-20 p-10 bg-opacity-10 bg-white bg-blur-md bg-clip-padding backdrop-filter backdrop-blur-lg backdrop-saturate-150 rounded-lg">
        <img class="text-white w-20 h-20 rounded-full" src="{{ $user->avatar_url }}" alt="imagen del Administrador">
        <div class="font-medium text-white">
            <div class="font-koulen">{{$user->name}}</div>
            <div class="font-lato text-sm text-gray-100 dark:text-gray-400">Bienvenido {{ $user->name}}</div>
        </div>
        <div>
